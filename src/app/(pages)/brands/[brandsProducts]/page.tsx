...
export default async function BrandsProductsPage({
  params,
}: {
  params: Params;
}) {
  const { brandsProducts } = await params;

  const response = await fetch(
    `${process.env.API_URL}/api/v1/products?brand=${brandsProducts}`,
    {
      next: { revalidate: 600 },
    },
  );
        ...
        <div className="flex items-end justify-between mb-3">
          <div>
            <h1 className="text-2xl md:text-3xl lg:text-4xl font-bold mb-2">
              {data?.data?.[0]?.brand?.name || "Products"}
            </h1>
      ...
      {data?.data?.length === 0 && (
        <div className="py-16 text-center">
          ...
          <p className="text-muted-foreground mb-8 max-w-md mx-auto">
            This brand does not have any products at the moment. Please check
            back later or browse other brands.
          </p>
